...
def build_prompt(all_data, subscriber):
    ...
    swing_section = (
        ...
        "STOCK: [ticker] | [BULLISH/BEARISH/NEUTRAL] | [news summary] | [swing setup] | [insider signal] | [thesis check]\n"
    ...
    if is_both:
        ...
        report_sections = scalp_section
    else:
        report_sections = swing_section

    prompt = (
        "You are a professional trading analyst AI writing a personalized morning report.\n"
        f"Today is {today}.\n"
        f"This report is for {name}, a {style_desc} watching: {tickers}.{focus_note}\n\n"
...
def sentiment_badge(s):
    ...
    if s == "BULLISH":
        return (
            ...
            "font-family:monospace;padding:2px 8px;border-radius:10px;font-weight:bold;'>"
            "&#x1F7E2; BULLISH</span>"
        )
    elif s == "BEARISH":
        return (
        ...
        return (
            "<span style='background:#fef9c3;color:#ca8a04;font-size:10px;"
            "font-family:monospace;padding:2px 8px;border-radius:10px;font-weight:bold;'>"
            "&#x1F7E1; NEUTRAL</span>"
        )
...
def build_html_email(report, subscriber):
    ...
    for line in lines:
        ...
        if line.startswith("SECTION:"):
            ...
            if any(x in title_upper for x in ["BIAS", "BREAKDOWN", "WEEKLY"]):
                accent = "#2a9fd6"
            elif any(x in title_upper for x in ["THREAT", "AVOID"]):
                accent = "#e63946"
            elif any(x in title_upper for x in ["CATALYST", "TIMES", "KEY", "CALENDAR"]):
                accent = "#f59e0b"
            elif in_part2:
                accent = "#e63946"
            else:
                accent = "#00c87a"
            body_html += (
                f"<div style='border-left:3px solid {accent};padding-left:12px;margin:18px 0 8px;'>"
                f"<div style='font-size:12px;font-weight:bold;color:{accent};font-family:monospace;letter-spacing:1.5px;'>"
                f"{title}</div></div>"
            )
            continue
        ...
        if line.startswith("STOCK:"):
            ...
            body_html += (
                f"<div style='border:1px solid {border};border-radius:5px;padding:10px 14px;background:{row_bg};'>"
                "<div style='display:flex;align-items:center;gap:10px;margin-bottom:6px;'>"
                f"<span style='font-size:14px;font-weight:bold;color:#2a9fd6;font-family:monospace;'>{ticker}</span>"
                f"{sentiment_badge(senti)}</div>"
                "<div style='font-size:12.5px;color:#374151;line-height:1.8;'>"
            )
            if col3:
                body_html += f"<b>{label3}:</b> {col3}<br/>"
            if col4:
                body_html += f"<b>{label4}:</b> {col4}<br/>"
            if col5:
                body_html += f"<b>{label5}:</b> {col5}<br/>"
            if col6:
                body_html += f"<b>Thesis:</b> {col6}"
            body_html += "</div></div>"
            continue
        ...
        if line.startswith("EVENT:"):
            ...
            if not in_event_table:
                body_html += (
                    ...
                    "style='border-collapse:collapse;font-size:12.5px;margin:6px 0;'>"
            ...
            body_html += (
                "<tr>"
                f"<td style='padding:6px 10px;color:#374151;border-bottom:1px solid #e2e8f0;"
                f"font-family:monospace;font-size:11px;'>{date}</td>"
                f"<td style='padding:6px 10px;color:#374151;border-bottom:1px solid #e2e8f0;'>{event}</td>"
                f"<td style='padding:6px 10px;color:#2a9fd6;border-bottom:1px solid #e2e8f0;"
                f"font-size:11px;'>{affects}</td>"
                "</tr>"
            )
            continue
    ...
    return (
        "<html><body style='margin:0;padding:0;background:#f0f2f5;font-family:Georgia,serif;'>"
        "<div style='max-width:680px;margin:24px auto;background:#ffffff;border-radius:8px;"
        "overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.1);'>"
        "<div style='background:#0d1117;padding:22px 28px 18px;'>"
        "<div style='font-size:11px;letter-spacing:3px;color:#00c87a;font-family:monospace;"
        "margin-bottom:6px;text-transform:uppercase;'>LIVE - " + today + "</div>"
        "<div style='font-size:24px;font-weight:bold;color:#ffffff;line-height:1.2;'>"
        "Morning Stock Report</div>"
        "<div style='font-size:11px;color:#6b7280;margin-top:5px;font-family:monospace;'>"
        "Hey " + name + " - your personalized report for: " + tickers + "</div>"
        "</div>"
        "<div style='padding:0 24px 28px;'>" + body_html + "</div>"
        ...
        "Sources: Yahoo Finance - Finnhub - AI: Groq / Llama 3.3 70B</div>"
        "<div style='font-size:11px;color:#374151;margin-top:4px;font-family:monospace;'>"
        "Not financial advice - Generated automatically - " + today + "</div>"
        "</div></div></body></html>"
    )
...
def send_email(report, subscriber):
    ...
    print(f"  Sent to {subscriber['name']} ({email})")
...
def main():
    ...
    print(f"Processing {len(subscribers)} subscriber(s)...")
    ...
    print(f"All {len(subscribers)} reports sent!")
